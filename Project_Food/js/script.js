...
window.addEventListener("DOMContentLoaded", () => {
    ...
    function setClock(selector, endtime) {
        ...
        updateClock(); // чтобы числа не моргали при обновлении страницы
    ...
    function bindPostData(form) {
        form.addEventListener("submit", (e) => {
            ...
            // 1 statusMessage ПОКАЗЫВАЕТ СПИННЕР, ОН ЗДЕСЬ РАСПОЛАГАЕТСЯ
            ...
            // 2 СОБИРАЕМ ВСЕ ДАННЫЕ ИЗ ФОРМЫ С ПОМОЩЬЮ FormData
            ...
            // 3 ОТПРАВЛЯЕМ ДАННЫЕ НА СЕРВЕР ПРИ ПОМОЩИ fetch
            /* fetch("server.php", {
                method: "POST", //показываем куда
                // header: {
                //     "Content-type": "application/json", //каким образом
                // },
                body: JSON.stringify(object), //и что именно отправляем

                //С ПОМОЩЬЮ ПРОМИСОВ ОБРАБАТЫВАЕМ РЕЗУЛЬТАТ ЗАПРОСА (load)
            }); */
            postData("http://localhost:3000/requests", json)
                // .then((data) => data.text()) //модифицируем данные в текст, т.к. без этого на сервер они не приходят
                .then((data) => {
                    console.log(data);
    ...
    function calcTotal() {
        //если хоть одного элемента нет, то считать не будем
